...
public class AutoRotateDayView extends View {
    ...
    public AutoRotateDayView(Context context) {
        super(context);
        this.mContext = context;
        initPainter();
    }

    public AutoRotateDayView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init(context, attrs);
        initPainter();
...
//        drawCirclePoint(canvas);
     ...
     */
    public AutoRotateDayView setTime(int day, int currentMonthAllDayCount) {
        if (day < 1 || day > 31 || currentMonthAllDayCount > 31 || currentMonthAllDayCount < 28) {
            Toast.makeText(getContext(), "天数-不合法", Toast.LENGTH_SHORT).show();
            return this;
        }
        dayArray = new String[currentMonthAllDayCount];
    ...
    private void drawHourNumber(Canvas canvas) {
        if (isDrawText) {
            if (dayArray==null||TextUtils.isEmpty(dayArray[0])) return;
            //旋转圆盘
            canvas.save();
